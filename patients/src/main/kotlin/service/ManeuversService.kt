...
object ManouversService {
    ...
    fun createDrug(routingContext: RoutingContext) {
        ...
        } else response.setStatusCode(HttpResponseStatus.BAD_REQUEST.code()).end()
    }
    ...
    var vertx: Vertx? = null
    private val MONGODB_CONFIGURATION = json {
        obj(
                "connection_string" to "mongodb://loveeclipse:PC-preh2019@ds149676.mlab.com:49676/heroku_jw7pjmcr"
        )
    }

    fun createSimpleManeuver(it: RoutingContext?) {
    }

    fun deleteSimpleManeuver(it: RoutingContext?) {
    }
}